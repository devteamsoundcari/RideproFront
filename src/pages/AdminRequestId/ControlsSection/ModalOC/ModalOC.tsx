            ...
            {allInstructors.map((instructor: any, idx) => {
              return (
                <tr key={idx}>
                  ...
                  <td>
                    {currencyFormatter(
                      instFares[instructor.instructors.id] - instructor.first_payment
                    )}
                  </td>
                  <td>
                    <Button
            ...
            {allProviders.map((provider: any, idx) => {
              return (
                <tr key={idx}>
                  ...
                  <td>
                    {currencyFormatter(provsFares[provider.providers.id] - provider.first_payment)}
                  </td>
                  <td>
                    <Button
                      variant="link"
                        ...
                        swal({
                          ...
                          if (willDelete) {
                            ...
                            if (res.status === 200) {
                              ...
                              swal(
                                'Tarifa Actualizada!',
                                `La tarifa de ${provider.providers.name} por ${currencyFormatter(
                      ...
                      swal({
                        ...
                        if (willDelete) {
                          ...
                          if (res.status === 200) {
                            swal(
                              'Tarifa Actualizada!',
                              `La tarifa de ${theTrack.contact_name} por ${currencyFormatter(
                                trackFare
                              )} fue actualizada 👍`,
                              'success'
                            );
      ...
      <Modal.Footer>
        ...
        <Button
          ...
          onClick={async () => {
            swal({
              title: '¿Estas seguro?',
              text: 'Si haces click confirmar cada proveedor recibira un email con el código de confirmación del pago y no habrá vuelta atrás!',
              icon: 'warning',
              ...
              if (willUpdate) {
                ...
                if (res.status === 200) {
                  ...
                  providers.forEach((item) => (item.hash = hashCode(item.id, requestId)));
                  instructors.forEach((item) => (item.hash = hashCode(item.id, requestId)));
